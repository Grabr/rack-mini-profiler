...
var MiniProfiler = (function () {
    ...
    var fetchResults = function (ids) {
        console.log("Fetching results for: " + ids);
        ...
        for (i = 0; i < ids.length; i++) {
            ...
            console.log("So we have an id: " + id);
            console.log("And we have fetchedIds: " + fetchedIds);
            console.log("And we have fetchingIds: " + fetchingIds);

            if ($.inArray(id, fetchedIds) < 0 && $.inArray(id, fetchingIds) < 0) {
                console.log("We get here if id is not in fetched or fetching arrays");
                idx = fetchingIds.push(id) - 1;
                console.log("Now we push id to fetchingIds and make request for results");
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        console.log("Successufuly fetched results");
                        fetchedIds.push(id);
                        if (json != "hidden") {
                            buttonShow(json);
                        }
                    ...
                    complete: function (json) {
                        console.log("That is complete, not success - button will not be shown");
                        console.log("But this is the json: " + json);
                        fetchingIds.splice(idx, 1);
                    },
                    error: function (json) {
                        console.log("If it is not success, it is error!!!!!");
                        console.log("And this is the error json: " + json);
                    }
                });
            }
        }
    ...
    var buttonShow = function (json) {
        ...
        console.log("Prepare to show the button");
        ...
        if (!controls && reqs > 1 && options.collapseResults && !expandedResults) {
          if (!totalsControl) {
            ...
            console.log("Appending stuff to dome");
    ...
    var initPopupView = function () {
        ...
        if (options.authorized) {
            ...
            console.log("Appending container 'profiler-results' to body form initPopupView() func");
            ...
            fetchTemplates(function () {
                ...
                console.log("Getting master page profiler results");
        ...
        // add support for the basic XMLHttpRequest object.
        if (typeof (XMLHttpRequest) != 'undefined') {
          console.log("Adding support for the basic XMLHttpRequest object");
          var _send = XMLHttpRequest.prototype.send;

          XMLHttpRequest.prototype.send = function sendReplacement(data) {
            if (this.onreadystatechange) {
              console.log("If we have onreadystatechange");
              if (typeof (this.miniprofiler) == 'undefined' || typeof (this.miniprofiler.prev_onreadystatechange) == 'undefined') {
                ...
                console.log("Adding current onreadystatechange to prev_onreadystatechange");
                this.onreadystatechange = function onReadyStateChangeReplacement() {
                  console.log("Ajax request in proggress to: " + this.responseURL);
                  if (this.readyState == 4) {
                    console.log("Ajax request finished to: " + this.responseURL);
                    console.log("Getting response headers");
                    var stringIds = this.getResponseHeader('X-MiniProfiler-Ids');
                    console.log("Response headers: " + stringIds);
                    if (stringIds) {
                      var ids = typeof JSON != 'undefined' ? JSON.parse(stringIds) : eval(stringIds);
                      console.log("Fetching results: " + ids);
                      fetchResults(ids);
                    }
                  }
